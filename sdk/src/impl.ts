...
export const ERROR_SPECIFIED_AMOUNT_RANGE =
  "specified amounts need to fit into int128";
export const ERROR_SPECIFIED_AMOUNT_MIXED_SIGN =
  "mixed exact-out / exact-in multi-hops";
...
export const ERROR_CALCULATED_TOKEN_MISMATCH =
  "last swaps of each multi-hop must end at the same calculated token";
export const ERROR_CALCULATED_AMOUNT_THRESHOLD_SIGN =
  "calculatedAmountThreshold sign and specified amount signs have to be equivalent";
export const ERROR_CALCULATED_AMOUNT_THRESHOLD_RANGE =
  "absolute value of calculatedAmountThreshold can't exceed maximum uint256 value";
export const ERROR_TOKEN0_TOKEN1_ORDER =
  "token0 and token1 are not ordered ascendingly";
export const ERROR_HOP_CONNECTION = "output of hop can't be used in next hop";
export const ERROR_UNDERLYING_EQ_WRAPPED =
  "underlying token must be different from wrapped token";
export const ERROR_SWAP_TYPE = "invalid swapType";
...
export function generateCalldataImpl(
  params: Parameters,
  { forceUnknownToken }: TestParameters = {
    forceUnknownToken: false,
  },
): Hex {
  const tokens = Tokens.load(params.chainId);
  const asUnknownToken = forceUnknownToken || tokens === null;

  const { multiHops, recipient, calculatedAmountThreshold, integrationFee } =
    params;
  const specifiedToken = getAddress(
    padHex(params.specifiedToken, { size: 20 }),
  );
  ...
  for (const { specifiedAmount, hops } of multiHops) {
    ...
    if (specifiedAmount !== 0n) {
      if (
        specifiedAmount < MIN_SPECIFIED_AMOUNT ||
        specifiedAmount > MAX_SPECIFIED_AMOUNT
      ) {
    ...
    for (let i = 0; i < hops.length; i++) {
      ...
      if (hop.type === "swap") {
        ...
        if (
          swapWithSqrtRatioLimit &&
          (sqrtRatioLimit < MIN_SQRT_RATIO ||
            sqrtRatioLimit > MAX_SQRT_RATIO ||
            (sqrtRatioLimit & NOT_BIT_MASK) < TWO_POW_62)
        ) {
          throw new Error(ERROR_INVALID_SQRT_RATIO_LIMIT);
        }

        if (
          typeof skipAhead === "number" &&
          (skipAhead < 0 || skipAhead > MAX_SKIP_AHEAD)
        ) {
  ...
  if (typeof calculatedAmountThreshold === "undefined") {
    ...
    const calculatedAmountThresholdAbs = isExactOutThreshold
    ...
    if (isExactOutThreshold) {
      calculatedAmountThresholdUnsigned =
        (calculatedAmountThresholdAbs + 1n) % TWO_POW_256;
    } else {
  ...
  const specifiedAmountBytes =
    maxSpecified === 0n ? 0 : size(numberToHex(maxSpecified));
  const withIntegrationFee =
    typeof integrationFee !== "undefined" && integrationFee.fee !== 0;
  ...
  for (const { specifiedAmount, hops } of multiHops) {
    calldata.push(
      padBytes(
        specifiedAmount === 0n
          ? new Uint8Array()
          : numberToBytes(abs(specifiedAmount)),
        { size: specifiedAmountBytes },
      ),
    ...
    for (let i = 0; i < hops.length; i++) {
      ...
      function maybePushHopCalculatedToken(hopCalculatedToken: Hex) {
        if (i !== hops.length - 1) {
          const swapCalculatedTokenId = asUnknownToken
            ? null
            : tokens.id(hopCalculatedToken);

          calldata.push(
            new Uint8Array([swapCalculatedTokenId ?? UNKNOWN_TOKEN]),
          );
      ...
      if (hop.type === "swap") {
        ...
        const [token0, token1] = [
          padHex(poolKey.token0, { size: 20 }),
          padHex(poolKey.token1, { size: 20 }),
        ];
        const [token0Big, token1Big] = [
          hexToBigInt(token0),
          hexToBigInt(token1),
        ];
        ...
        const [extension, fee, poolTypeConfig] = [
        ...
        function regularExtension() {
          calldata.push(new Uint8Array([1, skipAhead]), hexToBytes(config));
        }

        switch (hop.swapType) {
          case "base":
            calldata.push(
              new Uint8Array([0, skipAhead]),
              fee,
              poolTypeConfig,
            );
            break;
          ...
          case "ve33":
            ...
            calldata.push(
              new Uint8Array([4, skipAhead]),
              hexToBytes(extension),
              poolTypeConfig,
            );
            break;
          default:
            throw new Error(ERROR_SWAP_TYPE);
        }
        ...
        if (withSqrtRatioLimit) {
          ...
          if (typeof sqrtRatioLimitOpt === "undefined") {
            ...
            sqrtRatioLimit = isPriceIncreasing
              ? MAX_SQRT_RATIO
              : MIN_SQRT_RATIO;
          } else {
          ...
          calldata.push(
            numberToBytes(sqrtRatioLimit, { size: SQRT_RATIO_LIMIT_BYTES }),
          );
        }
      } else if (hop.type === "wrappedToken") {
        const [underlying, wrapped] = [
          padHex(hop.underlying, { size: 20 }),
          padHex(hop.wrapped, { size: 20 }),
        ];
        const [underlyingBig, wrappedBig] = [
          hexToBigInt(underlying),
          hexToBigInt(wrapped),
        ];
        ...
        calldata.push(new Uint8Array([3, Number(unwrap)]));
  ...
  const calculatedToken = getAddress(
    numberToHex(
  ...
  const calculatedAmountThresholdBin =
    calculatedAmountThresholdUnsigned === 0n
      ? new Uint8Array()
      : numberToBytes(calculatedAmountThresholdUnsigned);
  const [specifiedTokenId, calculatedTokenId] = asUnknownToken
    ? [null, null]
    : [tokens.id(specifiedToken), tokens.id(calculatedToken)];
  ...
  return bytesToHex(
    concatBytes([concatBytes(headerCalldata), concatBytes(calldata)]),
  );
}
